...
function getPathes(): Pathes {
  if (process.argv.length != 5) {
    throw Error(
      "Wrong command line arguments: usage <command> <input file path> <code file path> <comments file path>",
    );
  }
...
async function writeTofiles(
  ...
  const prComments = writeFile(commentsFile, comments).catch((error) => {
    throw Error(
      `writing to file with only comments with error: ${(error as Error).message}`,
    );
  });
  await Promise.all([prCode, prComments]);
}
